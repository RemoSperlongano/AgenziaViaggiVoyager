...
public class BoundaryVisitatoreSceltaTratta extends JFrame {
	/**
	 * 
	 */
	...
	private BoundaryVisitatoreSceltaTratta boundaryVisitatoreSceltaTratta = null;

	public static JPanel pannelloVisitatoreSceltaTratta;
	...
	public BoundaryVisitatoreSceltaTratta(){
		try {
		...
		boundaryVisitatoreSceltaTratta = this;
		...
		pannelloVisitatoreSceltaTratta = new JPanel();

		/* Impostazioni pannelloSelezionaViaggioOfferta */
		pannelloVisitatoreSceltaTratta.setSize(AABoundaryAvvio.Frame.getWidth(),
				AABoundaryAvvio.Frame.getHeight());
		AABoundaryAvvio.Frame.add(pannelloVisitatoreSceltaTratta);
		pannelloVisitatoreSceltaTratta.setLayout(null);
		...
		back.setLocation(500, 200);
		back.setSize(200, 35);
		back.setFont(new Font("Arial", 0, 20));

		/* Add dei pannelli al pannello principale */
		pannelloVisitatoreSceltaTratta.add(panelTitolo);
		pannelloVisitatoreSceltaTratta.add(panelComboBox);
	...
	private class GestoreComboBox implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == comboBoxAmbienti) {
				...
				try {
					selectedList = controlloreVisitatore
			...
			// Se la comboBox interessata è quella delle città di arrivo
				...
				if (!cittaArrivo.equals("--")) {
					...
					try {
						selectedList = controlloreVisitatore.estrazioneVia(
	...
	private class GestoreButtons implements ActionListener {
		public void actionPerformed(ActionEvent event) {
			if (event.getSource() == visualizzaOfferta) {
				...
				if (!controlloreVisitatore.verificaDati(listaCatalogo)) {
					areaVisualizzazione.setText("");
					areaVisualizzazione.append("Seleziona la tratta.");
				} else {
					pannelloVisitatoreSceltaTratta.setVisible(false);
					try {
						new BoundaryVisitatoreVisualizzaOfferta(
	...
	private void addComboBox(JComboBox<String> comboBox) {
		...
		try {
			selectedList = controlloreVisitatore.estrazioneAmbienti();
