 ...
 */
public class BoundaryVenditoreVisualizzaOfferta extends JFrame {
	/**
	 * 
	 */
	...
	public static JPanel pannelloVenditoreVisualizzaOfferta;
	...
	public BoundaryVenditoreVisualizzaOfferta(
		...
		pannelloVenditoreVisualizzaOfferta = new JPanel();

		pannelloVenditoreVisualizzaOfferta.setSize(
				AABoundaryAvvio.Frame.getWidth(),
				AABoundaryAvvio.Frame.getHeight());
		AABoundaryAvvio.Frame.add(pannelloVenditoreVisualizzaOfferta);
		pannelloVenditoreVisualizzaOfferta.setLayout(null);
		...
		pannelloVenditoreVisualizzaOfferta.add(panelTitolo);
		...
		pannelloVenditoreVisualizzaOfferta.add(panelVisualizza);
	...
	private class GestoreButtons implements ActionListener {
		...
		public void actionPerformed(ActionEvent event) {
			if (event.getSource() == visualizzaOfferte) {
				...
				if (!controlloreVenditore.verificaData(giorno.getText(),
						mese.getText())) {
					//Stampa di tutte le offerte disponibili in caso non si sia inserita la data.
					try {
						listaOfferte = controlloreVenditore.visualizzaOfferta(boundaryVenditoreOrdinaViaggi
					...
					try {
						listaOfferte = controlloreVenditore.visualizzaOffertaByData(
				...
				if (!controlloreVenditore.verificaId(offertaInserita.getText())) {
					areaVisualizzazione.append("\n"
							+ "Insersci id dell'offerta.");
				} else {
					try {
						pannelloVenditoreVisualizzaOfferta.setVisible(false);
						new BoundaryVenditorePrenotaViaggio(
								boundaryVenditoreOrdinaViaggi,
								boundaryVenditoreVisualizzaOfferta);
	...
	private class GestoreBack implements ActionListener {
		public void actionPerformed(ActionEvent arg0) {
			pannelloVenditoreVisualizzaOfferta.setVisible(false);
			BoundaryVenditoreOrdinaViaggi.pannelloVenditoreOrdinaViaggi
					.setVisible(true);
		}
	}
